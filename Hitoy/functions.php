...
//获取嵌套留言
function get_parent_comment($comment,$count=5){
    $count--;
    $allcom=new stdClass;
    $allcom->cid=$comment->comment_ID;
    $allcom->curl=$comment->comment_author_url;
    $allcom->cauthor=$comment->comment_author;
    $allcom->cdate=$comment->comment_date;
    $allcom->content=$comment->comment_content;
    $allcom->avatar=get_avatar($comment,96);
    $allcom->cpid=$comment->comment_parent;
    if($comment->comment_parent == 0 || $count < 0){
        return $allcom;
    }else{
        $pcomment=get_comment($comment->comment_parent,OBJECT);
        $allcom->parentcoment=get_parent_comment($pcomment);
        return $allcom;
    }
}
    ...
    if($_POST['action']=='asyn_loading_comments'){
        ...
        if($comments){
            ...
            foreach($comments as $comment){
                array_push($jsondata,get_parent_comment($comment));
            }
